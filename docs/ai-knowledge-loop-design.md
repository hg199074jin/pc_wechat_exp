...
`my_version` is a personal-interpretation derivative. `audit_case`, `sop`,
`prompt`, `faq`, `article`, and `script` are structured-rewrite derivatives
aligned with the existing conversion targets. The implementation validates this
explicit finite set; card types and derivative kinds are not interchangeable.

The current conversion endpoint becomes derivative creation. The original card's
`content_md` and `type` are no longer overwritten for new conversions.
...
`knowledge_derivatives.card_id` uses `ON DELETE CASCADE` because derivatives
cannot outlive their source card. `agent_rules.source_card_id` uses `ON DELETE
RESTRICT` because a published rule may still be in use. Card deletion is first
blocked by an application-level dependency check that returns a clear HTTP 409;
the database restriction is the final integrity safeguard for concurrent or
out-of-band deletion attempts.
...
The detail modal keeps its bounded viewport: `max-height: 80vh` and
`overflow-y: auto`. Rule Markdown textareas use a bounded initial height and vertical resize,
so long source evidence and editable rules remain inside the viewport on a
1080p screen.
...
- Database migration rerun: must be idempotent. The in-process initialization
  cache is schema-versioned and is marked current only after DDL plus migration
  commits, so an old user database is never skipped before `lifecycle_stage`
  exists.
- Sync interrupted: atomic file writes prevent partial generated files.

## 13. Test Plan

1. Migrate a database that contains current cards and verify all cards survive with `lifecycle_stage=captured`; run `init_db()` again and verify existing derivatives/rules remain unchanged.
